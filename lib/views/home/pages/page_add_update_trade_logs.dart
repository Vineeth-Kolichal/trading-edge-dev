                ...
                child: ElevatedButton(
                  onPressed: () async {
                    if (_formKey.currentState!.validate()) {
                      TradeLogsModel tradeLog = TradeLogsModel(
                        date: _selectedDate!,
                        type: type,
                        amount: pnlController.text,
                        description: commentController.text,
                        swingProfitCount: (swingProtController.text.isNotEmpty)
                            ? int.parse(swingProtController.text)
                            : 0,
                        swingLossCount: (swingLotController.text.isNotEmpty)
                            ? int.parse(swingLotController.text)
                            : 0,
                        intradayProfitCount:
                            (intraProController.text.isNotEmpty)
                                ? int.parse(intraProController.text)
                                : 0,
                        intradayLossCount: (intraLoController.text.isNotEmpty)
                            ? int.parse(intraLoController.text)
                            : 0,
                      );
                      if (widget.operation == 'Add') {
                        await tradeLogServices.addTradeLogs(tradeLog: tradeLog);
                        Get.back();
                      } else {
                        await tradeLogServices.updateTradeLogs(
                            documentId: widget.docId!,
                            updatedTradeLog: tradeLog);
